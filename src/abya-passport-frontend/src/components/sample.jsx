...
function Navbar({ currentPage, setCurrentPage }) {
  ...
  useEffect(() => {
    ...
    if (showConnectOptions) document.addEventListener("mousedown", onClick);
  ...
  return (
    <nav
      className={`fixed top-4 left-1/2 transform -translate-x-1/2 w-[95vw] max-w-5xl p-1 rounded-2xl z-50 transition-all duration-300 ${
        isScrolled
      ...
      <div className="bg-gradient-to-r from-blue-50/50 to-purple-50/50 rounded-xl p-4">
        <ul className="flex items-center justify-between gap-8">
          ...
          <li>
            <a
              href="/"
              className="text-2xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent transition-all duration-200 hover:from-blue-700 hover:to-purple-700"
            >
              ABYA Passport
            </a>
          </li>
          ...
          <li className="md:hidden">
            {/* insert your hamburger button here */}
          </li>
          ...
          <li className="relative" ref={dropdownRef}>
            {isAnyConnected ? (
              <div className="flex items-center gap-3">
                <div className="bg-gradient-to-r from-green-50 to-blue-50 border border-green-200 px-4 py-2 rounded-xl flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-gray-700">
                    {isConnected ? shorten(address) : shorten(principal)}
                  </span>
                ...
                >
                  ...
                  <svg width="20" height="20" fill="currentColor">
                    <path d="M12 8c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2zm0 2c-1.1 0-2
                     .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm0 6c-..."/>
                  </svg>
                </button>
              </div>
            ) : (
              <button
                onClick={() => setShowConnectOptions(true)}
                className="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-6 py-2.5 rounded-xl font-semibold flex items-center gap-2 transition-all duration-200 shadow-lg hover:shadow-xl"
              >
            ...
            {showConnectOptions && (
              <div className="absolute top-full mt-3 right-0 w-[300px] bg-white/95 backdrop-blur-xl p-6 rounded-2xl shadow-2xl border border-gray-200/20 animate-in slide-in-from-top-2 duration-200">
                {/* Header */}
                ...
                <div className="space-y-4">
                  ...
                  {isConnected && (
                    <div className="p-3 rounded-xl bg-green-50/50 border border-green-200/50">
                      ...
                      {didLoading ? (
                        <p className="mt-2 text-xs text-gray-500">Generating DID...</p>
                      ) : walletDid ? (
                        <div className="mt-2 text-xs">
                          <span className="font-mono break-all">{shorten(walletDid)}</span>
                        </div>
                      ) : null}
                      <button
                        onClick={() => disconnect()}
                        className="w-full mt-4 text-red-600 border border-red-200 px-2 py-2 rounded hover:bg-red-50"
                      >
                        Disconnect Wallet
                      </button>
                    </div>
