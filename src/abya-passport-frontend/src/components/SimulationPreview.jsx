...
function SimulationPreview({ onNotify, darkMode }) {
  ...
  const steps = [
    {
      id: "createDid",
      label: "Generating DID",
      duration: 1800,
      result: "did:ic:0xyZ...9Ab",
    },
    {
      id: "resolveDid",
      label: "Resolving DID",
      duration: 2700,
      result: "principal: abcdef-ghijk-lmnop",
    },
    {
      id: "issueVc",
      label: "Issuing Credential",
      duration: 2200,
      result: "vc-id: 0xCRED...123",
    },
    {
      id: "verifyVc",
  ...
  return (
    <div
      className={`w-full max-w-md h-96 border rounded-3xl p-5 shadow-2xl flex flex-col justify-between transition-colors duration-300 backdrop-blur-xl
      ${
        darkMode
          ? "bg-[#18181b] border-gray-100 text-yellow-100"
    ...
    >
      <div className="flex items-center justify-between mb-3">
        <div>
          <div
            className={`text-xs ${
              darkMode ? "text-yellow-300" : "text-slate-400"
            }`}
          >
            Flow simulation
          </div>
        ...
        <div
          className={`text-xs ${
            darkMode ? "text-yellow-200" : "text-slate-500"
          }`}
        >
          Demo mode
        </div>
      ...
      <div
        className={`h-36 rounded-lg p-3 flex flex-col justify-between border transition-colors duration-300
        ${
          darkMode
      ...
      >
        <div>
          ...
          {currentStep && (
            <div
              className={`mt-2 font-mono text-sm p-2 rounded break-words transition-colors duration-300
              ${
                darkMode
                  ? "text-yellow-100 bg-transparent"
        ...
        <div>
          <div
            className={`w-full rounded-full h-2 overflow-hidden mt-3 ${
              darkMode ? "bg-yellow-900/30" : "bg-gray-100"
            }`}
          >
            <div
              ...
              style={{
                width: `${progress}%`,
                background: `linear-gradient(90deg, ${ABYA.gold}, ${ABYA.brightGold})`,
          ...
          <div
            className={`mt-2 text-xs flex items-center justify-between ${
              darkMode ? "text-yellow-300" : "text-slate-500"
            }`}
          >
          ...
          .map((l) => (
            <div
              ...
              className={`text-xs flex items-center gap-3 ${
                darkMode ? "text-yellow-200" : "text-slate-600"
              }`}
            >
              <div
                className={`w-2 h-2 rounded-full ${
                  darkMode ? "bg-yellow-400" : "bg-emerald-300"
                }`}
              />
      ...
      <div className="mt-4 flex items-center justify-between gap-3">
        ...
        >
          This is a simulated demo — values are mock data for presentation only.
        </div>
